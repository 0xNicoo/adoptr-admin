...
import { getPostReportsByModelIdAction } from "@/actions/report"
...
const PostReports = () => {
    ...
    useEffect(() => {
        const fetchPostReports = async () => {
            try{
                const data = await getPostReportsByModelIdAction(post)
                setReports(data)
                setLoading(false)
            }catch(err){
                console.log(err)
    ...
    return(    
        <div className="min-h-screen bg-gray-100 p-8">
            <h1 className="text-4xl font-bold mb-4">Reportes de un post: {post}</h1>
